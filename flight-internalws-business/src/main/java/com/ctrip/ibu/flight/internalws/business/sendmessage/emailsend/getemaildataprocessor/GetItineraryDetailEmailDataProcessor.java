...
/**
 * 获取行程单邮件数据处理类Processor
 * Create by kyxie on 2018/4/16 14:50
 */
@Component
@GetEmailDataProcessor(name = "获取行程单邮件数据",processedEmailTypes = {EmailTemplateType.ItineraryDetail})
public class GetItineraryDetailEmailDataProcessor extends GetEmailDataProcessBase {

    private final static ILog CLOG = LogManager.getLogger(GetItineraryDetailEmailDataProcessor.class);
    ...
    public GetItineraryDetailEmailDataProcessor(IFlightCommonBusiness flightCommonBusiness,
    ...
    public GetEmailDataResponse getMessageSendInfo(GetEmailDataRequest getEmailDataRequest) throws GetEmailDataException {
        ...
        if (orderDetail == null){
            throw new GetEmailDataException(GetEmailDataResultCode.ORDER_DETAIL_INVALID,String.format("获取%d订单详情失败",sendEmailRequest.orderID));
        }
    ...
    private String getBodyContent(GetEmailDataRequest getEmailDataRequest) throws GetEmailDataException {
        ...
        try {
            ...
            throw new GetEmailDataException(GetEmailDataResultCode.TEMPLATE_RENDER_FAILED,String.format("渲染%s模板异常",emailTmplName));
        }
    }
    ...
    private List<EmailAttachmentDTO> getEmailAttachmentList(List<OrderDetailModel> orderDetailList, LanguageType emailSendLan, Map<String,String> additionalIds) {
        ...
        LoggerHelper.appendResponseContent("开始获取行程单附件...");
    ...
    private EmailAttachmentDTO getBilingualAttachment(String attachmentTmplName, List<OrderDetailModel> orderDetailList, LanguageType emailSendLan, Map<String,String> additionalIds){
        ...
        switch (attachmentTmplName){
            ...
            case QConfigConst.TEMPLATEFILENAME_ERECIPIENTATTACHMENT_SC:
                attachment.setAttachmentName(String.format("%s.pdf",messageResource.getResource(MessageResourceConst.MRKEY_ERECEIPT,emailSendLanLocale)));
                break;
            case QConfigConst.TEMPLATEFILENAME_ITINERARYATTACHMENT:
            case QConfigConst.TEMPLATEFILENAME_ITINERARYATTACHMENT_SC:
                attachment.setAttachmentName(String.format("%s.pdf",messageResource.getResource(MessageResourceConst.MRKEY_ITINERARY,emailSendLanLocale)));
                break;
        }
        ...
        try {
            ...
            if (!NO_NEED_MULTILAN_ORDERDETAIL_LANLIST.contains(emailSendLan) && orderDetailList.size() > 1){
                ...
                LoggerHelper.appendResponseContent(String.format("当前邮件发送语言%s需要获取多语言版附件\n",String.valueOf(emailSendLan)));
            ...
            LoggerHelper.addIndexedLogTag( IndexedLogTag.PdfGenerateResult,String.valueOf(PdfGenerateResult.SUCCESS));
    ...
    private String getTmplName(CorpGroup corpGroup){
        switch (corpGroup){
            case SC:
                return QConfigConst.TEMPLATEFILENAME_ITINERARY_SC;
            default:
                return QConfigConst.TEMPLATEFILENAME_ITINERARY;
        }
    }
