<#assign checkSeatInfoList=offlineValueAddedInfo.checkSeatInfoList>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>${offlineValueAddedInfo.statusDesc} - ${sharkMessageResource.offlineCheckSeat}</title>
    <style type="text/css">
        td[class="blue"]:hover {
            cursor: pointer;
                ...
                <tr>
                    <td style="padding:5px 20px 0;">
                        <table border="0" cellpadding="0" cellspacing="0" width="100%">
                            <tr>
                                <td style="font-family:Arial,sans-serif;font-size:32px;padding-bottom:20px;padding-top:20px;text-decoration:none;">${offlineValueAddedInfo.statusDesc} - ${sharkMessageResource.offlineCheckSeat}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <#if (offlineValueAddedInfo.flightColumnList?? && offlineValueAddedInfo.flightColumnList?size>=1)>
                                        ...
                                        <tr>
                                            <td valign="top"
                                                style="font-weight:700;border-bottom:1px solid #dddddd;padding:14px 10px 7px 0;">${sharkMessageResource.seatNo}</td>
                                            <td style="border-bottom:1px solid #dddddd;padding:14px 10px 7px;">
                                            <#list checkSeatInfoList as checkSeatInfo>
                                                <p style="margin:0;padding-bottom:10px;">${checkSeatInfo.passengerName}
                                                    <br/>
                                                ${checkSeatInfo.seatNumber},
                                                    <#if (checkSeatInfo.positionOptionNameList?? && checkSeatInfo.positionOptionNameList?size>=1)>
                                                        <#list checkSeatInfo.positionOptionNameList as positionOptionName>
                                                            <#if (positionOptionName_index < checkSeatInfo.positionOptionNameList?size-1)>
                                                            ${positionOptionName}|
                                                            <#else>
                                                            ${positionOptionName}
                                                            </#if>
