<#assign nameModifyInfoList = offlineValueAddedInfo.nameModifyInfoList>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>${offlineValueAddedInfo.statusDesc} - ${sharkMessageResource.offlineNameModify}</title>
    <style type="text/css">
        td[class="blue"]:hover {
            cursor: pointer;
                ...
                <tr>
                    <td style="padding:5px 20px 0;">
                        <table border="0" cellpadding="0" cellspacing="0" width="100%">
                            <tr>
                                <td style="font-family:Arial,sans-serif;font-size:32px;padding-bottom:20px;padding-top:20px;text-decoration:none;">${offlineValueAddedInfo.statusDesc} - ${sharkMessageResource.offlineNameModify}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <#if (nameModifyInfoList?? && nameModifyInfoList?size>=1)>
                                        <tr>
                                            <td valign="top"
                                                style="font-weight:700;border-bottom:1px solid #dddddd;padding:7px 10px 14px 0;">${sharkMessageResource.passenger}</td>
                                            <td style="border-bottom:1px solid #dddddd;padding:7px 0 14px 10px;font-size:14px;">
                                                <#list nameModifyInfoList as modifyNameInfo>
                                                    <#if (modifyNameInfo_index < nameModifyInfoList?size-1)>
                                                        <p style="color:#999999;margin:0;padding:0;">
                                                            <del style="color:#999999;">${modifyNameInfo.originalName}</del>
                                                        </p>
                                                    ${modifyNameInfo.modifiedName}
                                                        <br/>
                                                    <#else>
                                                        <p style="color:#999999;margin:0;padding:0;">
                                                            <del style="color:#999999;">${modifyNameInfo.originalName}</del>
